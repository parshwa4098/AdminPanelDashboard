...
export default function SettingsPage() {
 ...
 useEffect(() => {
  ...
  const handleRoleChange = (index: number, newRole: Userrole): void => {
  ...
  if (userString) {
    const loggedUser: User = JSON.parse(userString);
    if (loggedUser?.email === updatedUsers[index].email) {
      ...
      setUser({ ...loggedUser, role: newRole });
    }
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <select
          ...
          className={`
            w-[130px] p-2 rounded border outline-none cursor-pointer transition-colors text-sm font-medium
            ${
              u.role === "admin"
